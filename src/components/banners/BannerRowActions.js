...
export default function BannerRowActions({
  ...
  function handleUnpublish() {
    startTransition(async () => {
      const res = await unpublishBannerAction(bannerId);
      if (res?.error) {
        showError('Tắt slide thất bại', res.error);
        return;
      }
      showSuccess('Đã tắt slide');
      router.refresh();
    });
  }

  function handleDelete() {
    const label = banner?.banner_title || banner?.banner_placement || 'slide';
    if (!window.confirm(`Xóa slide "${label}"? Hành động không thể hoàn tác.`)) return;

    startTransition(async () => {
      const res = await deleteBannerAction(bannerId);
      if (res?.error) {
        showError('Xóa slide thất bại', res.error);
      }
    });
  }

  const editHref = `/banners/${bannerId}/edit`;
  const detailHref = `/banners/${bannerId}`;
